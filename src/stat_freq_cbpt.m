...
%% composite overview figure (6 bands x 11 times, SVG+PDF per condition)
% Rows:    (a) delta / (b) theta / (c) alpha / (d) beta / (e) low-gamma / (f) high-gamma
...
disp('--- creating 6-band x 11-time overview figures ---');
for ci = 1:length(conditions)
    cond      = conditions{ci};
    topo_imgs = cell(n_bands, n_times);

    for ri = 1:n_bands
        band_freq = BANDS_DEF{ri, 1};
        band_name = BANDS_DEF{ri, 2};
        band_dir  = fullfile(prj_dir, 'result', BANDS_DEF{ri, 3});
        ...
        fprintf('  [%s] %s band %d/%d done\n', cond, band_name, ri, n_bands);
    end
